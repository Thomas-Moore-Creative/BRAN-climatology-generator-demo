...
def main():
    ...
    logger.info("ARD - start write first zarr")
    BRAN2020_ard_path = '/scratch/es60/ard/reanalysis/BRAN2020/ARD/'
    ard_file_ID = 'BRAN2020-daily-temp-v07032024.zarr'
    DS.to_zarr(BRAN2020_ard_path+ard_file_ID,consolidated=True)
    logger.info("ARD - finish write first zarr & start reload")
    BRAN2020 = xr.open_zarr(BRAN2020_ard_path+ard_file_ID,consolidated=True)
    logger.info("ARD - CHUNK for time and WRITE zarr")
    ard_rcTime_file_ID = 'BRAN2020-daily-temp-chunk4time-v07032024.zarr'
    ...
    logger.info("ARD - finished with write rechunked zarr")
    # -------------
